                    ...
                    <el-form ref="form" :model="form" :rules="rules" hide-required-asterisk label-width="80px"
                        ...
                        <el-form-item label="模拟设备" prop="device">
                            <el-radio-group v-model="form.device">
                                ...
                                <el-radio-button label="android">Android 浏览器</el-radio-button>
                            </el-radio-group>
                        </el-form-item>
                        <el-form-item label="监测节点" prop="server">
                            <span v-if="serversInfo.length===0">暂无可用节点</span>
                            <el-radio-group v-else v-model="form.server">
                                <el-radio v-for="server in serversInfo" :key="server.addr"
                                          :label="server.addr">{{server.name}}
                                </el-radio>
                            </el-radio-group>
                        </el-form-item>
                    </el-form>
                    <div class="card-btn">
                        <el-button type="primary" plain circle icon="el-icon-arrow-right" @click="getData"></el-button>
                    </div>
                </div>
            </el-col>
        </el-row>
                ...
                <el-table :data="table" stripe border style="width: 100%">
                    ...
                    <el-table-column prop="whois" label="网址拥有者信息">
                        <template slot-scope="scope">
                            ...
                            <a v-else :href="handleWhoisUrl(scope.row.url)"
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    ...
                    device: [{required: true, message: '请选择模拟设备', trigger: 'change'}],
                    server: [{required: true, message: '请选择监测节点', trigger: 'change'}]
                },
                form: {
                    url: '',
                    device: 'pc',
        ...
        methods: {
            ...
            timeCheck(timestamp) {
                ...
                let time = updated.toLocaleString('zh-CN', {hour12: false});
                if (now - timestamp > 86400000)
                    return {status: false, time, day: parseInt((now - timestamp) / 86400000)}
                else return {status: true, time, day: 0}
            },
            // 更新监测节点信息
            updateServersInfo() {
                let info = localStorage.getItem("trackingUrl");
                if (info) {
                    ...
                    if (updateTime) {
                        let check = this.timeCheck(updateTime)
                        if (check.status) {
                            this.alertElement = {
                                type: 'success',
                        ...
                        this.alertElement = {
                            type: 'warning',
                            title: '距上次更新节点信息已过去很久，节点可能已经失效，请刷新页面更新'
                        }
                    }
                } else {
                    this.serversInfo = [];
                    ...
                    if (valid) {
                        ...
                        switch (this.form.device) {
                            case 'pc':
                                ...
                                break;
                            case 'wechat':
                                ua = 'Mozilla/5.0 (iPhone; CPU iPhone OS 13_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/7.0.8(0x17000820) NetType/WIFI Language/zh_CN';
                                break;
                            case 'iphone':
                                ...
                                break;
                            case 'android':
                                ua = 'Mozilla/5.0 (Linux; Android 7.0; Nexus 5X Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/54.0.2840.85 Mobile Safari/537.36';
                                break;
                            default:
                                ua = 'curl/7.54.0';
                                break;
                        ...
                        fetch(`http://${this.form.server}/v1/service/trackingUrl`, {
                            method: 'POST',
